  ...
  const generatePDF = (order = null) => {
    try {
      ...
      const tableRows = dataToPrint.map(o => [
        ...
        o.assignedVendor?.name || 'N/A',
        o.assignedDriver?.name || 'N/A',
        o.assignedDriver?.iqamaNumber || 'N/A',
        o.vehiclePlateNumber || 'N/A',
        o.status
      ]);

      autoTable(doc, {
        head: [['DN#', 'Route', 'Vendor', 'Driver Name', 'Iqama #', 'Plate #', 'Status']],
        body: tableRows,
        startY: 35,
        theme: 'grid',
                ...
                <tr key={order._id} className="transition-all hover:bg-primary/[0.02] dark:hover:bg-primary/[0.04]">
                  ...
                  <td className="p-6">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase ${
                      ...
                      order.status === 'Pending Approval' ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/20' : 
                    ...
                    <div className="flex justify-end gap-2">
                      {isAdmin && order.status === 'Pending' && (
                        <button 
                          ...
                          className="p-3 bg-green-50 dark:bg-green-900/20 text-green-600 hover:bg-green-500 hover:text-white rounded-xl transition-all shadow-sm"
                          title="Quick Confirm Delivery"
                        >
                          {isUpdating ? <Loader2 className="animate-spin" size={18}/> : <CheckCircle2 size={18} />}
                        </button>
                      )}
                      ...
                      {(isAdmin || currentUser?.permissions?.editDispatch) && (
                        <>
                          <button 
                            ...
                            className="p-3 bg-gray-50 dark:bg-gray-800 text-gray-400 hover:bg-blue-500 hover:text-white rounded-xl transition-all"
                            title="Edit"
                          >
                            <Edit2 size={18} />
                          </button>
                          {isAdmin && (
                            <button 
                              ...
                              className="p-3 bg-gray-50 dark:bg-gray-800 text-gray-400 hover:bg-red-500 hover:text-white rounded-xl transition-all"
                              title="Delete"
                            >
                              <Trash2 size={18} />
                            </button>
          ...
          <div className="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-8 bg-gray-50/50 dark:bg-gray-800/50 border-t flex justify-between items-center">
              ...
              <div className="flex gap-4">
                ...
                {/* Pending Status - Actions based on role/permission */}
                {selectedOrder.status === 'Pending' && (
                  <div className="flex gap-3">
                    {isAdmin && (
                      <button 
                    ...
                    {(isAdmin || currentUser?.permissions?.editDispatch) && (
                      <button 
                        ...
                        className="px-6 py-3.5 bg-blue-500 text-white rounded-2xl text-[10px] font-black uppercase shadow-lg shadow-blue-200 flex items-center gap-2 hover:scale-[1.02] active:scale-95 transition-all"
                      >
                        <Edit2 size={16}/> Edit
                      </button>
                    )}
                    ...
                    {isAdmin && (
                      <button 
                        ...
                        className="px-6 py-3.5 bg-red-500 text-white rounded-2xl text-[10px] font-black uppercase shadow-lg shadow-red-200 flex items-center gap-2 hover:scale-[1.02] active:scale-95 transition-all"
                      >
