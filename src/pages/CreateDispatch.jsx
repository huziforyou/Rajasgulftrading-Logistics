...
const CreateDispatch = () => {
  ...
  const handlePdfPaste = (text) => {
    ...
    try {
      ...
      const material = findField([/Material[: ]*([^\n\r]+)/i, /Description[: ]*([^\n\r]+)/i, /Items[: ]*([^\n\r]+)/i]);
      const quantity = findField([/Qty[: ]*([0-9.]+)/i, /Quantity[: ]*([0-9.]+)/i, /Total Qty[: ]*([0-9.]+)/i]);
      const dn = findField([/DN[: ]*([A-Z0-9-]+)/i, /Delivery Note[: ]*([A-Z0-9-]+)/i, /DN Number[: ]*([A-Z0-9-]+)/i]);
      const customer = findField([/Customer[: ]*([^\n\r]+)/i, /Customer Name[: ]*([^\n\r]+)/i, /Client[: ]*([^\n\r]+)/i]);
      const loadingDateRaw = findField([/Date[: ]*([0-9\-\/]+)/i, /Loading Date[: ]*([0-9\-\/]+)/i, /DN Date[: ]*([0-9\-\/]+)/i]);
      const origin = findField([/From[: ]*([^\n\r]+)/i, /Loading From[: ]*([^\n\r]+)/i, /Origin[: ]*([^\n\r]+)/i]);
      const destination = findField([/To[: ]*([^\n\r]+)/i, /Offloading To[: ]*([^\n\r]+)/i, /Destination[: ]*([^\n\r]+)/i]);

      if (material) setValue('materialDescription', material);
      if (quantity) setValue('materialQuantity', quantity);
      if (dn) setValue('deliveryNoteNumber', dn);
      if (customer) setValue('customerName', customer);
      if (origin) setValue('loadingFrom', origin);
      if (destination) setValue('offloadingTo', destination);
      ...
      if (loadingDateRaw) {
        ...
        if (parts.length === 3) {
          ...
          if (formattedDate) setValue('loadingDate', formattedDate);
        }
      }
    ...
    const initData = async () => {
      ...
      try {
        ...
        if (isEditMode) {
          ...
          reset({
            loadingDate: new Date(data.loadingDate || data.loadingDateTime).toISOString().split('T')[0],
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        ...
        {!isEditMode && (
          <motion.div 
            ...
            className="glass-card p-6 border-2 border-dashed border-primary/20 bg-primary/5 rounded-[2rem]"
          >
            <div className="flex flex-col md:flex-row gap-6 items-center">
              <div className="flex-1 space-y-2 w-full">
                <div className="flex items-center gap-2 mb-2">
                  <FileText size={16} className="text-primary" />
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">PDF Quick Parse (Paste Text)</h4>
                </div>
                <textarea 
                  className="w-full h-24 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 text-xs font-medium focus:ring-2 focus:ring-primary/20 outline-none transition-all no-scrollbar"
                  placeholder="Paste text from PDF here (Material, DN, Date, etc.)..."
                  onChange={(e) => handlePdfPaste(e.target.value)}
                ></textarea>
                <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest ml-1">
                  * System will automatically detect fields like DN Number, Material, and Customer.
                </p>
              </div>
              {isParsing && (
                <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm animate-pulse">
                  <Loader2 size={14} className="animate-spin text-primary" />
                  <span className="text-[10px] font-black uppercase tracking-widest text-primary">Parsing...</span>
                </div>
              )}
            </div>
          </motion.div>
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            >
              <div className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Loading Date</label>
                  <div className="relative">
                    <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                    <input type="date" {...register('loadingDate', { required: true })} className="input-field pl-12" />
                  </div>
                </div>
